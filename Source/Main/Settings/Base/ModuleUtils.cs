...
public static class ModuleUtils
{
    ...
    public static void TechLevelMenuWithExtraOption(this Listing_Standard listing, string extraOptionName, int extraIdValue, string name, string explanation, TechLevel value, float position, float buttonWidth, Action<TechLevel> action)
    {
        ...
        if (action != null && Widgets.ButtonText(new Rect(position - buttonWidth, rect.y, buttonWidth, 29f), (int)value == extraIdValue ? extraOptionName : ArmorSettings.GetTechLevelName(value), true, true, true, null))
        {
            ...
            foreach (var enumValue in Enum.GetValues(typeof(TechLevel)))
                list.Add(new FloatMenuOption(ArmorSettings.GetTechLevelName((TechLevel)enumValue), () => action.Invoke((TechLevel)enumValue), MenuOptionPriority.Default, null, null, 0f, null, null, true, 0));
    ...
    public static void TechLevelMenu(this Listing_Standard listing, string name, string explanation, TechLevel value, float position, float buttonWidth, Action<TechLevel> action)
    {
        ...
        if (action != null && Widgets.ButtonText(new Rect(position - buttonWidth, rect.y, buttonWidth, 29f), ArmorSettings.GetTechLevelName(value), true, true, true, null))
        {
            List<FloatMenuOption> list = new List<FloatMenuOption>();
            foreach (var enumValue in Enum.GetValues(typeof(TechLevel)))
                list.Add(new FloatMenuOption(ArmorSettings.GetTechLevelName((TechLevel)enumValue), () => action.Invoke((TechLevel)enumValue), MenuOptionPriority.Default, null, null, 0f, null, null, true, 0));
