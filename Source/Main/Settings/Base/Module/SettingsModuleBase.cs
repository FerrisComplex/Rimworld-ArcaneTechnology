...
{
    public void Look<T>(ref T value, string label, T defaultValue = default(T), bool forceSave = false) => Scribe_Values.Look(ref value, "DFerrisArcaneTech_" + (_cachedModuleCatgory.NullOrEmpty() ? "MissingCategory_" : _cachedModuleCatgory.Replace(" ", "") + "_") + label.Replace(" ", ""), defaultValue, forceSave);
